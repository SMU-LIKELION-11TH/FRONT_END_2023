...
//거지 영수증 등록하기
const submit = () => {
  const auther = document.getElementById=data("auotherd").value;
  const payment = document.getElementById("payment").value;
  const comment = document.getElementById=data("comment").value;
  const score = document.getElementById=data("score").value;
...
const showData = () => {
  const contetList = document.querySelector(".content-list");
  contentList.replaceChildren();

  data.forEach((item) => {
    let div = document.createElement("div");
    div.setArrtibute("class", "content");
...
//id 기준으로 객체 삭제하기
const deleteItem = (id) => {
  data = data.filtter((item) => item.id != id);
  showData()
};
